...
var (
	...
	attackValues          []int    //攻击装备数值
	defenseEquipmentNames []string //防御装备名字数组
	defenseValues         []int    //防御装备数值
	magicEquipmentNames   []string //魔法装备名字数组
	magicAttack           []int    //魔法装备攻击数值
	magicDefense          []int    //魔法装备防御数值
...
func initFileConfigs() {
	...
	//初始化攻击装备数值数组
	attackValues = cfg.Section("equipment").Key("attackValues").Ints(delimiter)
	//初始化防御装备名称数组
	defenseEquipmentNames = cfg.Section("equipment").Key("defenseNames").Strings(delimiter)
	//初始化防御装备数值数组
	defenseValues = cfg.Section("equipment").Key("defenseValues").Ints(delimiter)
	//初始化魔法装备名称数组
	magicEquipmentNames = cfg.Section("equipment").Key("magicNames").Strings(delimiter)
	//初始化防御装备数值数组
	magicAttack = cfg.Section("equipment").Key("magicAttack").Ints(delimiter)
	//初始化防御装备数值数组
	magicDefense = cfg.Section("equipment").Key("magicDefense").Ints(delimiter)
}

//初始化装备数据
func InitEquipments() {
	//初始化攻击装备
	for i, name := range attackEquipmentNames {
		AttackEquipment[name] = &model.Equipment{
			Name:        name,
			AttackValue: getValueByIndex(i, attackValues),
		}
	}
	//初始化防御装备
	for i, name := range defenseEquipmentNames {
		DefenseEquipment[name] = &model.Equipment{
			Name:         name,
			DefenseValue: getValueByIndex(i, defenseValues),
		}
	}
	//初始化魔法装备
	for i, name := range magicEquipmentNames {
		MagicEquipment[name] = &model.Equipment{
			Name:         name,
			AttackValue:  getValueByIndex(i, magicAttack),
			DefenseValue: getValueByIndex(i, magicDefense),
		}
	}
}
...
func InitRoles() {
	...
	for _, name := range roleNames {
		Roles[name] = &model.Role{
			Name:        name,
			AttackValue: utils.RandInt(attackValue[0], attackValue[1]), //随机生成攻击力
			BloodValue:  utils.RandInt(bloodValue[0], bloodValue[1]),   //随机生成血量
		}
	}
...
//从数组中获取值，先判断index是否越界
func getValueByIndex(i int, values []int) (atk int) {
	if len(values) >= i {
		atk = values[i]
	}
	return
